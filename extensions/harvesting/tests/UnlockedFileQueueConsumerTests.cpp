...
#define TEST_CLASS UnlockedFileQueueConsumerTests
	...
	namespace {
		void AssertFailureWhenDecryptedDataHasInvalidSize(size_t dataSize) {
			...
			auto saltedEncrypted = test::GenerateEphemeralAndEncrypt(clearText, recipientKeyPair.publicKey());

			// Act:
			auto decryptedPair = TryDecryptUnlockedEntry(saltedEncrypted, recipientKeyPair);
